...
from app.database.models.assessment import TopicTest, Attendance
...
# Импорты схем
from app.schemas.assessment.test_result import (
    TestInfoResponse,
    TestSessionResponse,
    TestResultResponse,
    TestsListResponse,
    TestSessionFullResponse,
    TestResultFullResponse,
                ...
                for block in blocks:
                    ...
                    for topic in topics:
                        ...
                        try:
                            ...
                            training_attempts = [
                                TestAttemptResponse(
                                    topic_test_id=test.topic_test_id,
...
@router.post("/session", 
             response_model=TestSessionFullResponse,
...
async def create_test_session(
    ...
    try:
        ...
        from app.schemas.assessment.tests import SessionProgressResponse
        
        return TestSessionFullResponse(
            session=session,
...
@router.post("/session/{session_id}/finish", 
             response_model=TestResultFullResponse,
...
async def finish_test(
    ...
    try:
        ...
        for question in session.questions:
            ...
            from app.schemas.assessment.tests import DetailedTestResultResponse
            detailed_results.append(DetailedTestResultResponse(
...
@router.get("/student/{student_id}/attendance",
            ...
            description="Получение записей посещаемости студента")
async def get_student_attendance(
    ...
    try:
        ...
        if subject_id:
            attendances = await get_attendance_by_student_and_subject_db(db, student_id, subject_id)
        else:
            from sqlalchemy import select
            result = await db.execute(
                select(Attendance).filter(Attendance.student_id == student_id)
            )
...
@router.get("/student/{student_id}/attendance/statistics",
            response_model=AttendanceStatisticsResponse,
            summary="Получить статистику посещаемости",
            description="Получение статистики посещаемости студента")
async def get_attendance_statistics(
    ...
    try:
        stats = await get_attendance_statistics_db(db, student_id, subject_id)
        return AttendanceStatisticsResponse(**stats)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка получения статистики посещаемости: {str(e)}"
        )

# ===========================================
# КОММЕНТАРИИ К СТУДЕНТУ
# ===========================================

@router.get("/student/{student_id}/comments",
            response_model=List[CommentResponse],
            summary="Получить комментарии студента",
            description="Получение всех комментариев к студенту")
...
@router.get("/student/{student_id}/comments/statistics",
            response_model=CommentStatisticsResponse,
            summary="Получить статистику комментариев",
            description="Получение статистики комментариев студента")
async def get_comments_statistics(
    ...
    try:
        stats = await get_comment_statistics_db(db, student_id=student_id)
        return CommentStatisticsResponse(**stats)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка получения статистики комментариев: {str(e)}"
        )
...
@router.get("/student/{student_id}/final-grade/{subject_id}",
            response_model=FinalGradeResponse,
            summary="Получить итоговую оценку",
            description="Получение итоговой оценки студента по предмету")
async def get_final_grade(
    ...
    try:
        grade_data = await calculate_final_grade_db(db, student_id, subject_id)
        return FinalGradeResponse(**grade_data)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка получения итоговой оценки: {str(e)}"
        )
